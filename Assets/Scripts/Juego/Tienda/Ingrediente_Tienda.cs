...
public class Ingrediente_Tienda : MonoBehaviour
{
    ...
    {
        ver_Ingrediente();
        tienda.comprarIngrediente(ingrediente, nivel_desbloqueo);
    }
    ...
    private void cargar_Ingrediente()
    {
        ...
        if(ingrediente != null)
        {
            gameobject_boton_compra.transform.GetChild(0).gameObject.GetComponent<Text>().text = ingrediente.valor.ToString() + " €";
            imagen.sprite = ingrediente.sprite;
        }
    }
    ...
    {
        if (ingrediente != null && dinero >= ingrediente.valor) comprable = true;
